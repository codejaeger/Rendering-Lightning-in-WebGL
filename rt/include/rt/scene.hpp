...
namespace rt
{
 ...
 class scene_t
 {
   ...
   private:
    ...
    ///Parse a tmesh object
    object_t* parse_object_tmesh(XMLElement* _elm, const std::list<material_t*>& matlist);
    /// Parse a string value
    std::string parse_string(XMLElement* _elm, std::string _property);
    ///Returns a string value property
    std::string parse_string(std::string _val);
    ...
    /// Parse the textured image
    image_t* parse_tex_image(XMLElement* _elm,std::string property);

    ///
    image_t* parse_tex_image(std::string property);
